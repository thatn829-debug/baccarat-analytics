...
# =========================================================================
# SYSTEM CORE v16.1: MAXIMUM COMBINATORIAL & QUANTUM INTEGRITY ENGINE
# =========================================================================
def calculate_baccarat_v16_quantum(p_cards, b_cards, shoe_history, shoe_decks=8, 
    ...
    if detailed_cards_count > 0:
        ...
        mode = "TỔ HỢP CHUỖI PHI LẶP BIẾN THIÊN ĐA TẦNG (CORE v16.1)"
    else:
        ...
        if cards_removed == 0 and manual_games_played > 0:
            # Nếu chỉ nhập tổng số ván chạy mà không nhập chi tiết, tự động ước tính số lá bài trung bình đã dùng
            cards_removed = int(manual_games_played * 4.85)
    ...
    # 3. MÔ HÌNH DỰ ĐOÁN TỔ HỢP HOÀN CHỈNH THEO QUY TẮC BỐC LÁ THỨ 3 CHUẨN QUỐC TẾ
    if len(p_cards) == 2 and p_score >= 6:  # Player đứng bài
    ...
    elif len(p_cards) == 2:  # Player bắt buộc phải rút lá thứ 3
...
# =========================================================================
st.set_page_config(page_title="Oracle Ultimate v16.1", page_icon="🔮", layout="centered")
...
st.markdown(
    ...
    .discrepancy-warning { background-color: rgba(241, 196, 15, 0.1); border: 1px solid #f1c40f; color: #f1c40f; font-size: 11px; padding: 6px; border-radius: 4px; margin-top: 6px; font-weight: 600;}
...
# Kiểm tra lệch dữ liệu nhưng KHÔNG khóa app
is_data_discrepancy = (manual_games > 0 and manual_games != calculated_total_wins)
...
if st.session_state.last_results:
    ...
    if isinstance(results_data[0], str) and results_data[0].startswith("❌"):
        ...
        with right_pair_col:
            ...
            # Thẩm định tính toàn vẹn bộ bài
            if is_shoe_logical:
            ...
            # Xu hướng chuỗi chữ cái + Cảnh báo dạng cầu
            if st.session_state.outcome_history or is_data_discrepancy:
                trend_letters = []
                ...
                trend_html_str = " ".join(trend_letters) if trend_letters else "<span style='color:#666; font-style:italic;'>Trống</span>"
                ...
                p_live_count = st.session_state.outcome_history.count("Player")
                b_live_count = st.session_state.outcome_history.count("Banker")
                p_live_pct = round((p_live_count / total_live_games) * 100, 1) if total_live_games > 0 else 0.0
                b_live_pct = round((b_live_count / total_live_games) * 100, 1) if total_live_games > 0 else 0.0
                
                discrepancy_html = ""
                if is_data_discrepancy:
                    discrepancy_html = f"""
                    <div class="discrepancy-warning">
                        ⚠️ Cảnh báo cấu hình Sidebar chưa đồng bộ: Thiết lập {manual_games} ván thắng nhưng tổng số ván lẻ (P+B+T) mới nhập là {calculated_total_wins} ván. 
                        Vui lòng cập nhật đủ số ván thắng lẻ của Player, Banker và Tie để đạt độ chính xác tối cao.
                    </div>
                    """
                ...
                st.markdown(
                    f"""
                    <div class="trend-hud">
                        <div class="trend-title">📈 XU HƯỚNG ĐƯỜNG ĐI BÀI THỰC TẾ ({total_live_games} ván live)</div>
                        ...
                        <div style="font-size: 11px; color: #aaa; margin-top: 6px; font-family: monospace;">
                            Thực tế live: Player {p_live_pct}% | Banker {b_live_pct}%
                        </div>
                    </div>
                    """, 
                    unsafe_allow_html=True
...
btn_trigger = st.button("🚀 GHI NHẬN VÀ TÍNH TOÁN VÁN TIẾP THEO", use_container_width=True, type="primary")
...
if btn_trigger:
    ...
    if not p_input.strip() and not b_input.strip():
        ...
        if p_list or b_list:
            ...
            # Kích hoạt bộ sinh tính toán tối cao Core v16.1
