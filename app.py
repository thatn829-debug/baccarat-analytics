...
# =========================================================================
# SYSTEM CORE v18.2: ULTRA QUANTUM ENGINE (CACHED & OPTIMIZED FOR SPEED)
# =========================================================================
# Thêm Cache để tránh tính toán lại các tổ hợp Markov/Bayes trùng lặp
@st.cache_data(max_entries=20, ttl=300)
def calculate_baccarat_v18_ultimate(p_cards, b_cards, shoe_history_tuple, shoe_decks=8, 
                                    manual_cards_used=0, manual_games_played=0,
                                    p_wins=0, b_wins=0, tie_wins=0):
    
    # Chuyển đổi tuple ngược lại thành list để xử lý logic bên trong
...
# Cache tĩnh UI để trình duyệt không tốn tài nguyên render lại CSS
@st.cache_data
...
# =========================================================================
st.set_page_config(page_title="Oracle Engine v18.2 Ultra Fast", page_icon="🔮", layout="centered")
inject_custom_css()
...
if st.sidebar.button("🔄 RESET TOÀN BỘ KHAY BÀI", use_container_width=True):
    ...
    st.cache_data.clear() # Xóa sạch bộ nhớ đệm khi reset bài
    st.rerun()

# --- ĐIỀU KHIỂN NHẬP TAY (TỐI ƯU FORM KHÔNG CẦN RERUN) ---
st.subheader("🃏 Nhập Dữ Liệu Dự Đoán Ván Tiếp Theo")

with st.form(key="baccarat_input_form", clear_on_submit=True):
    col_p, col_b = st.columns(2)
    with col_p: p_input = st.text_input("PLAYER (Lá bài vừa ra):", placeholder="Ví dụ: 5,K,2")
    with col_b: b_input = st.text_input("BANKER (Lá bài vừa ra):", placeholder="Ví dụ: J,7")
...
if submit_button:
    ...
    if not p_input.strip() and not b_input.strip():
        ...
        if p_list or b_list:
            # Ép shoe_history thành dạng Tuple để làm khóa cho @st.cache_data (vì List không hash được)
            shoe_history_tuple = tuple(st.session_state.shoe_history)
            
            core_output = calculate_baccarat_v18_ultimate(
                p_list, b_list, shoe_history_tuple, shoe_decks=decks,
    ...
    if st.session_state.last_results:
        ...
        if isinstance(results_data, str):
            ...
            with right_pair_col:
                ...
                else: st.markdown(f'<div class="validation-hud logic-fail">⚠️ LỖI LOGIC: ÂM KHAY BÀI ({", ".join(invalid_cards)})</div>', unsafe_allow_html=True)
