...
# =========================================================================
# GIAO DIỆN & STYLE CHUYÊN BIỆT CHO MOBILE
# =========================================================================
st.set_page_config(page_title="Oracle Engine v18.2 Mobile", page_icon="🔮", layout="centered")

# Thêm CSS hack giúp tinh gọn font chữ và thu nhỏ lề trên màn hình điện thoại
st.markdown(
    """
    <style>
    /* Thu nhỏ padding mặc định của Streamlit trên di động */
    ...
    /* Tối ưu hóa HUD box trên màn hình nhỏ */
    .hud-box { padding: 12px; border-radius: 6px; text-align: center; margin-bottom: 8px; border: 1px solid #4f4f4f; background-color: #1a1a1a; }
    .hud-title { font-size: 11px; font-weight: 600; color: #b0b0b0; letter-spacing: 0.3px; }
    .hud-value { font-size: 24px; font-weight: 800; font-family: monospace; margin-top: 2px; }
    
    .neon-player-advantage { background-color: #0984e3 !important; border: 1px solid #74b9ff !important; box-shadow: 0 0 10px rgba(9, 132, 227, 0.5); }
    .neon-banker-advantage { background-color: #d63031 !important; border: 1px solid #ff7675 !important; box-shadow: 0 0 10px rgba(214, 48, 49, 0.5); }
    .neon-tie-alert { border: 1.5px solid #2ecc71 !important; box-shadow: 0 0 10px rgba(46, 204, 113, 0.5); }
    
    .validation-hud { padding: 8px; border-radius: 4px; text-align: center; font-weight: 700; font-size: 12px; font-family: monospace; margin-bottom: 8px; }
    .logic-pass { background-color: rgba(46, 204, 113, 0.15); border: 1px solid #2ecc71; color: #2ecc71; }
    .logic-fail { background-color: rgba(231, 76, 60, 0.15); border: 1px solid #e74c3c; color: #e74c3c; }
    
    .trend-hud { padding: 10px; border-radius: 6px; background-color: #151515; border: 1px dashed #444; }
    .trend-title { font-size: 10px; font-weight: bold; color: #888; text-transform: uppercase; margin-bottom: 4px;}
    .trend-string { font-size: 15px; font-family: monospace; letter-spacing: 4px; font-weight: 800; margin-bottom: 4px; white-space: nowrap; overflow-x: auto; }
    .trend-alert { font-size: 12px; margin-top: 4px; }
    .char-p { color: #54a0ff; } .char-b { color: #ff7675; } .char-t { color: #2ecc71; }
    
    /* Ép text input gọn gàng hơn trên di động */
    div[data-testid="stTextInput"] label { font-size: 12px !important; margin-bottom: 2px !important; }
...
# =========================================================================
# PHẦN NHẬP ĐIỂM: CHIA CỘT NGANG TỐI ƯU CHO SMARTPHONE
# =========================================================================
st.write("📋 **Nhập bài ván vừa ra:**")

# Sử dụng tỷ lệ 1:1 chia đều 2 cột theo hàng ngang hoàn hảo trên Mobile
col_p, col_b = st.columns(2)
with col_p: 
    p_input = st.text_input("🔵 PLAYER:", value="", placeholder="Ví dụ: 5,K,2")
with col_b: 
    b_input = st.text_input("🔴 BANKER:", value="", placeholder="Ví dụ: J,7")
...
calc_triggered = st.button("🚀 GHI NHẬN & DỰ ĐOÁN TIẾP", use_container_width=True, type="primary")
...
if is_strict_lock:
    st.error(f"🛑 Lỗi đồng bộ số liệu ván thắng lẻ ở Sidebar!")
else:
    if st.session_state.last_results:
        ...
        if isinstance(results_data, str):
            ...
            st.write("🔮 **Kết Quả Quét Khay Bài:**")
            
            # Chia cột ngang cho khối hiển thị kết quả chính nhằm tiết kiệm không gian màn hình dọc
            ...
            with right_col:
                # Thu nhỏ thông tin cửa đôi bằng cách đặt chung hàng
                c1, c2 = st.columns(2)
                with c1: st.metric("🔵 P.Pair", f"{p_pair}%")
                with c2: st.metric("🔴 B.Pair", f"{b_pair}%")
                
                if is_shoe_logical: 
                    st.markdown('<div class="validation-hud logic-pass">✔ KHAY HỢP LỆ</div>', unsafe_allow_html=True)
                else: 
                    st.markdown('<div class="validation-hud logic-fail">⚠️ LỖI KHAY BÀI</div>', unsafe_allow_html=True)
                ...
                if st.session_state.outcome_history:
                    ...
                    st.markdown(f'<div class="trend-hud"><div class="trend-string">{" ".join(trend_letters)}</div><div class="trend-alert" style="color: {pattern_color}; font-weight: bold;">{pattern_msg}</div></div>', unsafe_allow_html=True)

            st.markdown("---")
            total_shoe_cards = decks * 52
            penetration_rate = min(100.0, (((total_shoe_cards - max(0, cards_left))) / total_shoe_cards) * 100)
            st.caption(f"**Chế độ:** `{mode}` | **Độ chín:** {round(penetration_rate, 1)}%")
            st.progress(penetration_rate / 100.0)
    else:
        st.info("🔮 Vui lòng nạp quân bài ván vừa ra để tính toán.")
