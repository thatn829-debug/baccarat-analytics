...
# =========================================================================
# SYSTEM CORE v11.0: 2D COMBINATORIAL CONVOLUTION ENGINE (COSMIC MAX)
# =========================================================================
def calculate_baccarat_v11_core(p_cards, b_cards, shoe_history, shoe_decks=8, 
    ...
    # 1. KHẤU TRỪ CHUẨN XÁC THEO LỊCH SỬ THỰC TẾ TRƯỚC
    if detailed_cards_count > 0:
        ...
        mode = "MA TRẬN TÍCH CHẬP 2 CHIỀU ĐA BIẾN (MỨC TỐI HẬU V11)"
    else:
    ...
    # 2. PHÂN TÍCH MA TRẬN 2D KHÔNG HOÀN LẠI TOÀN PHẦN (LÁ 3 PLAYER & LÁ 3 BANKER CÙNG LÚC)
    if p_score >= 6:  # Player không rút
        ...
        for card3_p in range(10):
            ...
            # Khấu trừ động khay bài ngay sau khi Player lấy lá 3
            ...
            if b_draws:
                # DỰNG MA TRẬN TÍCH CHẬP 2 CHIỀU ĐỂ TÍNH BIẾN ĐỘNG PHỤ THUỘC TUYỆT ĐỐI
                for card3_b in range(10):
                    # Khấu trừ chuẩn xác: Nếu Banker rút trúng loại lá bài Player vừa rút trước đó
                ...
                # Banker dừng bài ở lá thứ 2, so điểm trực tiếp với 3 lá Player
...
# =========================================================================
# LÕI PHÂN TÍCH XU HƯỚNG VÀ CHUỖI (PATTERN ENGINE)
# =========================================================================
...
# =========================================================================
st.set_page_config(page_title="Oracle Cosmic Edge v11.0", page_icon="🔮", layout="centered")
...
st.sidebar.header("⚙️ CẤU HÌNH HỆ THỐNG V11.0")
...
if is_data_discrepancy:
    ...
    **⚠️ Yêu cầu:** Vui lòng điều chỉnh lại dữ liệu ở Sidebar sao cho hai số liệu này trùng khớp nhau.
    ...
    if st.session_state.last_results:
        ...
        if isinstance(results_data[0], str) and results_data[0].startswith("❌"):
            ...
            with k_col2:
                ...
                if "Fibonacci" in capital_strategy or "Martingale" in capital_strategy:
                    st.markdown("**Kết quả ván vừa rồi để cược ván sau:**")
                    f_win, f_lose = st.columns(2)
                    if f_win.button("👍 THẮNG VÁN TRƯỚC", use_container_width=True):
                        st.session_state.fibo_index = max(0, st.session_state.fibo_index - 2)
                        st.session_state.martingale_step = 1
                        st.rerun()
                    if f_lose.button("👎 THUA VÁN TRƯỚC", use_container_width=True):
        ...
        st.info("🔮 Vui lòng nhập dữ liệu ván bài ở ô bên dưới để kích hoạt hệ thống phân tích v11.0.")
...
# --- ACTION TRIGGER ---
btn_trigger = st.button("🚀 KÍCH HOẠT HỆ THỐNG V11.0 COSMIC CORE", use_container_width=True, type="primary", disabled=is_data_discrepancy)
...
if btn_trigger and not is_data_discrepancy:
    ...
    if not p_input.strip() and not b_input.strip():
        ...
        if p_list or b_list:
            ...
            core_output = calculate_baccarat_v11_core(
