...
# =========================================================================
# SYSTEM CORE v19.0: MAXIMUM COMBINATORIAL EXACT ENGINE (SUPREME LIVE UPDATE)
# =========================================================================
def calculate_baccarat_v18_ultimate(p_cards, b_cards, shoe_history, shoe_decks=8, 
    ...
    # Thiết lập khay bài nguyên bản gồm 13 loại quân (1 đến 13), mỗi loại có 4 * số bộ bài
    ...
    if detailed_cards_count > 0:
        ...
        mode = "SIÊU TỔ HỢP TỔ TRƯỜNG PHI HOÀN LẠI TOÀN PHẦN (TỐI CAO)"
    else:
        # Nếu không có lịch sử chi tiết, tính lượng bài đã xả dựa trên ước lượng thống kê tối ưu
        ...
        mode = "PHÂN RÃ ĐA PHƯƠNG TRÌNH BAYES ĐỒNG NHẤT" if cards_removed > 0 else "KHAY BÀI NGUYÊN BẢN (XÁC SUẤT GỐC)"
    ...
    # Kiểm tra tính logic của khay bài âm
    invalid_cards_list = []
    ...
    # Khấu trừ các lá bài đã lật trên bàn ván hiện tại ra khỏi khay bài trước khi tính xác suất lá tiếp theo
    for card in p_cards + b_cards:
        if card in deck_structure and deck_structure[card] > 0:
            deck_structure[card] -= 1

    # Chuyển đổi cấu trúc khay bài sang dạng điểm số Baccarat (Quân bài >= 10 có giá trị là 0)
    score_deck = [0.0] * 10
    for card_num, count in deck_structure.items():
        if card_num >= 10: 
            score_deck[0] += count
    ...
    # --- TÍNH TOÁN CHÍNH XÁC PLAYER PAIR & BANKER PAIR (MỨC ĐỘ SÂU) ---
    ...
    b_pair_prob = sum((deck_structure[i] / N_total) * ((deck_structure[i] - 1) / (N_total - 1)) for i in range(1, 14) if deck_structure[i] >= 2)
    b_pair_odds = round(b_pair_prob * 100, 4)

    # Hoàn trả lại các lá bài hiện tại vào cấu trúc hiển thị khay bài của giao diện để không lệch báo cáo
    for card in p_cards + b_cards:
        if card in deck_structure:
            deck_structure[card] += 1

    # Tính điểm số hiện tại của Player và Banker trên bàn
    p_score = sum([0 if c >= 10 else c for c in p_cards]) % 10
    b_score = sum([0 if c >= 10 else c for c in b_cards]) % 10

    # Khối kiểm tra điều kiện thắng tự nhiên (Natural 8, 9) hoặc cả hai bên đều đã đủ 3 lá
    if (len(p_cards) == 2 and p_score >= 8) or (len(b_cards) == 2 and b_score >= 8) or (len(p_cards) == 3 and len(b_cards) == 3):
    ...
    # --- THUẬT TOÁN QUÉT KHÔNG GIAN MẪU TỔ HỢP TỐI ĐA ---
    player_wins, banker_wins, ties = 0.0, 0.0, 0.0

    # Trường hợp 1: Player ĐỨNG (6 hoặc 7 điểm). Banker bốc nếu điểm <= 5
    ...
    # Trường hợp 2: Player BẮT BUỘC BỐC LÁ THỨ 3 (0 đến 5 điểm)
    elif len(p_cards) == 2:
        for card3_p in range(10):
            ...
            # Khấu trừ lá thứ 3 của Player ra khỏi không gian mẫu trước khi tính lá thứ 3 của Banker
            score_deck[card3_p] -= 1
            N1 = N_total - 1.0
            
            # Áp dụng chính xác quy tắc bốc lá thứ 3 quốc tế của Banker dựa trên lá thứ 3 của Player
            b_draws = False
            ...
            # Hoàn trả trạng thái không gian mẫu phục vụ lượt quét tổ hợp tiếp theo
            score_deck[card3_p] += 1
...
# =========================================================================
# INTERFACE DESIGN & STYLES (GIỮ NGUYÊN GIAO DIỆN)
# =========================================================================
